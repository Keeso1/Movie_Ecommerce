...
export default async function MoviePage(props: { params: { id: string } }) {
    ...
    const directors = movie.moviePersons.filter(person => person.role.toLowerCase() === 'director');
    const actors = movie.moviePersons.filter(person => person.role.toLowerCase() === 'actor');
                ...
                <div className="flex flex-col justify-center">
                    <div className="flex flex-col">
                        ...
                        <div className="flex flex-row justify-between">
                            <p className="text-gray-700 text-lg mb-6">{new Date(movie.releaseDate).toLocaleDateString()}</p>
                            ...
                            <div className="mb-6">
                                ...
                                <div className="flex flex-wrap gap-x-4 gap-y-2">
                                    {directors.map(director => (
                                        <span key={director.id} className="text-lg text-gray-800">{director.name}</span>
                                    ))}
                                </div>
                            </div>
                        )}
                        ...
                        {actors.length > 0 && (
                            <div className="mb-6">
                                ...
                                <div className="flex flex-wrap gap-x-4 gap-y-2">
                                    {actors.map(actor => (
                                        <span key={actor.id} className="text-lg text-gray-800">{actor.name}</span>
                                    ))}
                                </div>
                            </div>
                        )}
                            ...
                            <div className="mb-6">
                                ...
                                <div className="flex flex-wrap gap-x-4 gap-y-2">
                                    {movie.genres.map(genre => (
                                        <span key={genre.id} className="text-lg text-gray-800">{genre.name}</span>
                                    ))}
                                </div>
                            </div>
                        )}
                        ...
                        <div className="flex justify-between items-center mt-4">
                            <span className="text-2xl font-semibold text-green-600">${movie.price.toFixed(2)}</span>
                        </div>
                    </div>
                </div>
            </div>
